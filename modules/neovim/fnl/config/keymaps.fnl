...
(let [keymap (require :lib.keymap)
  ...
  (if vim.env.ZELLIJ
      (let [zj (require :lib.zellij)]
        ...
        (keymap.map :<c-h> :<cmd>ZellijNavigateLeftTab<cr> { :desc "Navigate left"})
        (keymap.map :<c-l> :<cmd>ZellijNavigateRightTab<cr> { :desc "Navigate right"})
        (keymap.map :<c-j> :<cmd>ZellijNavigateDown<cr> { :desc "Navigate down"})
        (keymap.map :<c-k> :<cmd>ZellijNavigateUp<cr> { :desc "Navigate up"})
        ; tabs
        (keymap.map :<c-w>zt #(zj.new-tab) {:desc "New zellij tab"})
        ; panes
        (keymap.map :<a-n> #(zj.new-pane))
        (keymap.map :<c-w>zp #(zj.new-pane) {:desc "New pane"})
        ; pane sizing
        ...
        (keymap.map :<c-w>zl #(zj.increase :right) {:desc "Increase pane right"})
        (keymap.map :<c-w>zL #(zj.decrease :right) {:desc "Decrease pane right"})
        (keymap.map :<c-w>zk #(zj.increase :up) {:desc "Increase pane up"})
        (keymap.map :<c-w>zK #(zj.decrease :up) {:desc "Decrease pane up"})
        ;; misc.
        (keymap.map :<a-g> #(zj.run.run zj.lazygit)))
      (do (keymap.map :<C-h> :<C-w>h {:desc "Navigate left"})
  ...
  (keymap.map :S #(plugin-utils.flash.treesitter) {:desc "Flash Treesitter" :mode [:n :x :o]})
  (let [fzf-lua (require :lib.fzf-lua)]
    ...
    (keymap.map :<leader>/ #(fzf-lua.grep-visual) {:mode :v :desc "Grep selection"})
